...
from src.environment import OdysseyEnvironment

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler("odyssey.log"),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Odyssey - Adversarial Training System")
...
def main():
    ...
    try:
        ...
        env.tcp_client.host = args.labyrinth_host
        env.tcp_client.port = args.labyrinth_port
        
        logger.info(f"Connecting to Labyrinth at {args.labyrinth_host}:{args.labyrinth_port}")
